...
describe('isOpaqueElement', () => {
	...
	test('returns false for transparent background', async () => {
		const tree = await setContent(`<div id="transparent" style="background: transparent;">Transparent</div>`);
	...
	test('returns false for rgba with low alpha', async () => {
		const tree = await setContent(`<div id="semi" style="background: rgba(255, 0, 0, 0.5);">Semi</div>`);
	...
	test('returns true for rgba with high alpha', async () => {
		const tree = await setContent(`<div id="opaque-rgba" style="background: rgba(255, 0, 0, 0.95);">Opaque RGBA</div>`);
