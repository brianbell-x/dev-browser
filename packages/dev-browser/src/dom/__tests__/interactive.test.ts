...
describe('isInteractive', () => {
	...
	test('role="textbox" elements are interactive', async () => {
		const tree = await setContent(`<div role="textbox" id="role-textbox" contenteditable="true">Text</div>`);
	...
	test('role="checkbox" elements are interactive', async () => {
		const tree = await setContent(`<div role="checkbox" id="role-checkbox" aria-checked="false">Check</div>`);
	...
	test('role="slider" elements are interactive', async () => {
		const tree = await setContent(`<div role="slider" id="role-slider" aria-valuenow="50">Slider</div>`);
	...
	test('large iframes are interactive', async () => {
		const tree = await setContent(`<iframe id="large-iframe" src="about:blank" width="200" height="200"></iframe>`);
	...
	test('small iframes are not interactive', async () => {
		const tree = await setContent(`<iframe id="small-iframe" src="about:blank" width="50" height="50"></iframe>`);
